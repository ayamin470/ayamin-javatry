...
public class Step05ClassTest extends PlainTestCase {
    ...
    // done ayamin 修行#++: 継承を使わないやり方もどこかでやってみましょう by jflute (2025/09/05)
    ...
    // done jflute インターフェースを使った実装にしようと思っていますが、アリですか？(step05でインターフェースって学んでいましたっけ？) by ayamin (2025/10/06)
    // インターフェースにしちゃうと、「Ticketは普通のクラスになって」という条件を満たせないから意味ないかも
    // done ayamin [へんじ] interfaceは当然step5では登場してないけど、でも使ってもOK。もう修行レベルだしね by jflute (2025/10/22)
    ...
    // hint2: チケット種別という概念を、種別要素一つ一つをクラスで表現するのがあやみんさんの方法だった...
    //  e.g. OneDayクラス、TwoDayクラス...
    // チケット種別を class ただ一つで表現するというhint1の話...
    // クラスは一個だから種別要素をクラスで表現はできない。クラスの中にはどんな文法がある？
    // チケット種別に対して、種別要素を複数(OneDay, TwoDay...)、つまり one-to-manyの関係性。
    // クラスに対して、複数になる文法は？ (振る舞い=メソッドも複数定義できる、属性=インスタンス変数も複数定義できる)
